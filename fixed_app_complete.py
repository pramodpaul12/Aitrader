...
# The rest of app.py remains the same...from trading_engine_instance import TradingEngine
from db_manager import DatabaseManager
from utils import is_asx_trading_hours, get_asx_market_status, format_currency, calculate_performance_metrics
from logger_config import setup_logger
...
db_manager = DatabaseManager()
analyzer = StockAnalyzer()
trading_engine_instance = TradingEngine(data_service, analyzer)
...
with tab1:
    ...
    if stop_button:
        ...
        if st.session_state.current_position:
            try:
                trading_engine_instance.close_position(
    ...
    if is_asx_trading_hours():
            ...
            try:
                ...
                if st.session_state.current_position:
                    ...
                    if current_price <= take_profit_target:
                        pnl = trading_engine_instance.close_position(position, "Take profit", current_price)
                    ...
                    if current_price >= stop_loss_target:
                        pnl = trading_engine_instance.close_position(position, "Stop loss", current_price)
                    ...
                    # Check if it's time for a cycle change and we're still in the same position
                        ...
                        pnl = trading_engine_instance.close_position(position, "End of cycle rotation", current_price)
                ...
                # If no open position and it's time for a new cycle, find a new stock to short
                    ...
                    if best_stock and best_score >= 60:  # Only short if score is at least 60
                        ...
                        # Open position
                        try:
                            position = trading_engine_instance.open_position(symbol, position_size, current_price)
        ...
        if st.session_state.current_position:
            try:
                ...
                current_price = data_service.get_latest_stock_data(symbol)['last_price']
                pnl = trading_engine_instance.close_position(
                    st.session_state.current_position, 
                    "Market closed",
